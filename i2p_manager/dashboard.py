...
from .lib.i2pd_control import I2PdController
from .lib.firefox_profile import FirefoxProfileManager
...
class Dashboard:
    def __init__(self):
        ...
        self.i2pd = I2PdController(self.config_manager)
        self.firefox = FirefoxProfileManager(self.config_manager)
        self.running = True
    ...
    def render_header(self) -> Panel:
        """Render the header"""
        title = Text("I2P EASY MANAGER v0.1.0", style="bold white on blue", justify="center")
        return Panel(title, border_style="blue")
    
    def render_status(self) -> Panel:
        """Render connection status"""
        if not self.status_data:
            return Panel("[yellow]Checking status...[/yellow]", title="Connection Status", border_style="cyan")
        ...
        if self.status_data.get('running'):
            ...
            content.append(f"Router: ", style="white")
            content.append("Running\n", style="green")
            
            config = self.config_manager.load_config()
            proxy_addr = f"{config['i2pd']['host']}:{config['i2pd']['http_port']}"
            content.append(f"Proxy: ", style="white")
            content.append(f"{proxy_addr}\n", style="cyan")
            content.append(f"Console: ", style="white")
            content.append(f"http://127.0.0.1:{config['i2pd']['console_port']}", style="cyan")
        else:
            content.append(f"\nStatus: ", style="white")
    ...
    def render_network(self) -> Panel:
        ...
        content.append(f"Active Tunnels: ", style="white")
    ...
    def render_footer(self) -> Panel:
        """Render footer with last action"""
        text = Text("Press 1-8 to select an action, R to refresh, Q to quit", 
    ...
    def update_display(self, layout: Layout):
        ...
        layout["footer"].update(
            Layout(self.render_menu(), size=10)
        )
    
    async def update_status(self):
        """Update I2P status data"""
        try:
            self.status_data = self.i2pd.get_status()
        except Exception:
    ...
    def action_start(self):
        ...
        try:
            ...
            config = self.config_manager.load_config()
            self.firefox.launch(config['firefox']['profile_name'])
    ...
    def action_config(self):
        ...
        console.print(f"  {self.config_manager.get_config_path()}")
...
def launch_dashboard():
    ...
    import termios if sys.platform != 'win32' else None
    import tty if sys.platform != 'win32' else None
    
    dashboard = Dashboard()
    
    # Initial status update
    import asyncio
    asyncio.run(dashboard.update_status())
