...
def run(managers, force=False):
    ...
    with Progress(
        ...
        if not i2pd_installed:
            progress.update(task, description="[yellow]I2Pd not found (optional)[/yellow]")
        ...
        progress.update(task, description=f"[green]Profile created: {profile['name']}[/green]")
        ...
        if cfg['firefox']['harden_with_arkenfox']:
            ...
            progress.update(task, description="[green]Arkenfox settings applied[/green]")
